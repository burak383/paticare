...
// No real audio files ship with this demo, so these are named presets the
// backend just stores as a string preference — actual sound playback would
// come from expo-notifications' Android channel / iOS sound config once a
// real audio asset is added.
...
export default function ProfileScreen() {
  const { user, logout, biometricSupported, biometricEnabled, setBiometricEnabled } = useAuth();
  ...
  async function saveEdit() {
    ...
    try {
      ...
      if (editAvatarUri) patch.avatarUrl = editAvatarUri;
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Pressable style={styles.plusCard} onPress={() => Alert.alert('PatiCare Plus', 'Plus üyeliği yakında açılıyor.')}>
          ...
          <View style={styles.plusCopy}>
            ...
            <Text style={styles.plusDescription}>Sınırsız tarama geçmişi, gelişmiş etkileşim kontrolleri ve PDF sağlık raporları.</Text>
      ...
      <Modal visible={!!editingPet} transparent animationType="slide" onRequestClose={() => setEditingPet(null)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Pressable style={styles.editPhotoRow} onPress={handleChangeEditPhoto} disabled={pickingEditPhoto} testID="edit-pet-photo-button">
              <View style={styles.editPhotoAvatar}>
                {editAvatarUri || editingPet?.avatarUrl ? (
...
const styles = StyleSheet.create({
  ...
  editPhotoRow: { marginTop: 12, flexDirection: 'row', alignItems: 'center', gap: 12 },
