...
export default function CalendarScreen() {
  ...
  async function runAction(item: CareItem, action: 'complete' | 'skip' | 'undo') {
    ...
    try {
      const updated =
        action === 'complete'
          ? await careItemsApi.completeCareItem(item.id)
          : action === 'skip'
            ? await careItemsApi.skipCareItem(item.id)
            : await careItemsApi.undoCareItem(item.id);
      setDayItems((prev) => prev.map((c) => (c.id === updated.id ? updated : c)));
      ...
      if (action === 'undo') {
        if (remindersEnabled && selectedPet) scheduleCareItemReminder(updated, selectedPet.name).catch(() => {});
      } else {
  ...
  async function handleCreateReminder() {
    ...
    try {
      ...
      if (remindersEnabled) scheduleCareItemReminder(created, selectedPet.name).catch(() => {});
    } catch (err) {
      Alert.alert('Hata', err instanceof Error ? err.message : 'Hatırlatıcı eklenemedi.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.calendarCard}>
          ...
          <View style={styles.daysRow}>
            {week.map((day) => {
              ...
              return (
                <Pressable key={day.toISOString()} style={styles.day} onPress={() => setSelectedDate(day)}>
      ...
      <Pressable
        ...
        onPress={() => (selectedPet ? setModalVisible(true) : Alert.alert('Önce bir evcil hayvan ekle'))}
      >
      ...
      <Modal visible={modalVisible} transparent animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalSubtitle}>{selectedDayLabel} için</Text>
            ...
            <View style={styles.modalActions}>
              ...
              <Pressable style={styles.modalSubmit} onPress={handleCreateReminder} disabled={submitting} testID="reminder-submit">
                {submitting ? <ActivityIndicator color={colors.primaryForeground} /> : <Text style={styles.modalSubmitText}>Ekle</Text>}
              </Pressable>
            </View>
          </View>
        </View>
