...
export default function HealthRecordScreen() {
  ...
  const [addVetNoteVisible, setAddVetNoteVisible] = useState(false);
  const [editingVetNote, setEditingVetNote] = useState<VetNote | null>(null);
  const [vetNoteVetName, setVetNoteVetName] = useState('');
  const [vetNoteText, setVetNoteText] = useState('');
  const [savingVetNote, setSavingVetNote] = useState(false);
  ...
  function handleDeleteVaccine(vaccine: Vaccine) {
    Alert.alert('Aşı kaydını sil', `${vaccine.title} kaydı silinsin mi?`, [
      { text: 'Vazgeç', style: 'cancel' },
      {
        text: 'Sil',
        style: 'destructive',
        onPress: async () => {
          try {
            await healthApi.deleteVaccine(vaccine.id);
  ...
  function handleEditConditionPress(condition: Condition) {
    setEditingCondition(condition);
    setConditionTitle(condition.title);
    setConditionNote(condition.note || '');
    setAddConditionVisible(true);
  }

  async function handleSubmitCondition() {
    if (!selectedPet) return;
    if (!conditionTitle.trim()) {
      Alert.alert('Eksik bilgi', 'Rahatsızlık adı gerekli.');
      return;
    }
    setSavingCondition(true);
    try {
      if (editingCondition) {
        const updated = await healthApi.updateCondition(editingCondition.id, {
          title: conditionTitle.trim(),
          note: conditionNote.trim(),
        });
        setConditions((prev) => prev.map((c) => (c.id === updated.id ? updated : c)));
      } else {
        const created = await healthApi.addCondition(selectedPet.id, {
          title: conditionTitle.trim(),
      ...
      Alert.alert('Hata', err instanceof Error ? err.message : 'Rahatsızlık kaydedilemedi.');
    } finally {
      setSavingCondition(false);
    }
  }

  function handleDeleteCondition(condition: Condition) {
    Alert.alert('Rahatsızlık kaydını sil', `${condition.title} kaydı silinsin mi?`, [
      { text: 'Vazgeç', style: 'cancel' },
      {
        text: 'Sil',
        style: 'destructive',
        onPress: async () => {
          try {
            await healthApi.deleteCondition(condition.id);
  ...
  function closeVetNoteModal() {
    setAddVetNoteVisible(false);
    setEditingVetNote(null);
    setVetNoteVetName('');
    setVetNoteText('');
  }

  function openAddVetNoteModal() {
    setEditingVetNote(null);
    setVetNoteVetName('');
    setVetNoteText('');
    setAddVetNoteVisible(true);
  }

  function handleEditVetNotePress(note: VetNote) {
    setEditingVetNote(note);
    setVetNoteVetName(note.vetName || '');
    setVetNoteText(note.note);
    setAddVetNoteVisible(true);
  }

  async function handleSubmitVetNote() {
    if (!selectedPet) return;
    if (!vetNoteText.trim()) {
      Alert.alert('Eksik bilgi', 'Not metni gerekli.');
      return;
    }
    setSavingVetNote(true);
    try {
      if (editingVetNote) {
        const updated = await healthApi.updateVetNote(editingVetNote.id, {
          vetName: vetNoteVetName.trim() || undefined,
          note: vetNoteText.trim(),
        });
        setVetNotes((prev) => prev.map((n) => (n.id === updated.id ? updated : n)));
      } else {
        const created = await healthApi.addVetNote(selectedPet.id, {
          vetName: vetNoteVetName.trim() || undefined,
          note: vetNoteText.trim(),
        });
        setVetNotes((prev) => [...prev, created]);
      }
      closeVetNoteModal();
    } catch (err) {
      Alert.alert('Hata', err instanceof Error ? err.message : 'Not kaydedilemedi.');
    } finally {
      setSavingVetNote(false);
    }
  }

  function handleDeleteVetNote(note: VetNote) {
    Alert.alert('Veteriner notunu sil', 'Bu not silinsin mi?', [
      { text: 'Vazgeç', style: 'cancel' },
      {
        text: 'Sil',
        style: 'destructive',
        onPress: async () => {
          try {
            await healthApi.deleteVetNote(note.id);
            setVetNotes((prev) => prev.filter((n) => n.id !== note.id));
          } catch (err) {
            Alert.alert('Hata', err instanceof Error ? err.message : 'Silinemedi.');
          }
        },
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView
        testID="health-record-scroll"
        contentContainerStyle={styles.scrollContent}
        showsVerticalScrollIndicator={false}
        refreshControl={<RefreshControl testID="health-record-refresh-control" refreshing={loading} onRefresh={load} tintColor={colors.primary} />}
      >
              ...
              {vaccines.length === 0 ? (
                ...
                vaccines.map((vaccine) => (
                  ...
                  >
                    ...
                    <Pressable
                      hitSlop={8}
                      style={styles.vaccineDeleteButton}
                      onPress={() => handleDeleteVaccine(vaccine)}
                      testID={`delete-vaccine-${vaccine.id}`}
                    >
                      <Icon name="trash-2" size={17} color={colors.mutedForeground} />
                    </Pressable>
                  </Pressable>
                ))
              )}
        ...
        {filter === 'Rahatsızlıklar' ? (
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              ...
              <Pressable style={styles.addButton} testID="add-condition-button" onPress={openAddConditionModal}>
                <Icon name="plus" size={16} color={colors.primary} />
                <Text style={styles.addText}>Kayıt ekle</Text>
              </Pressable>
            </View>
              ...
              conditions.map((condition) => (
                <View key={condition.id} style={styles.cardRow}>
                  ...
                  <Pressable
                    ...
                    onPress={() => handleEditConditionPress(condition)}
                    testID={`edit-condition-${condition.id}`}
                    accessibilityLabel={`${condition.title} kaydını düzenle`}
                  >
                  ...
                  <Pressable
                    ...
                    testID={`delete-condition-${condition.id}`}
                    accessibilityLabel={`${condition.title} kaydını sil`}
                  >
        ...
        {filter === 'Veteriner notları' ? (
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              ...
              <Pressable style={styles.addButton} testID="add-vet-note-button" onPress={openAddVetNoteModal}>
                <Icon name="plus" size={16} color={colors.primary} />
                <Text style={styles.addText}>Not ekle</Text>
              </Pressable>
            </View>
            {vetNotes.length === 0 ? (
              <View style={styles.cardWithPadding}>
              ...
              vetNotes.map((note) => (
                <View key={note.id} style={styles.cardWithPadding}>
                  <View style={styles.doctorRowHeader}>
                    ...
                    <View style={styles.rowIconGroup}>
                      <Pressable
                        hitSlop={8}
                        style={styles.rowIconButton}
                        onPress={() => handleEditVetNotePress(note)}
                        testID={`edit-vet-note-${note.id}`}
                        accessibilityLabel="Veteriner notunu düzenle"
                      >
                        <Icon name="edit-2" size={16} color={colors.mutedForeground} />
                      </Pressable>
                      <Pressable
                        hitSlop={8}
                        style={styles.rowIconButton}
                        onPress={() => handleDeleteVetNote(note)}
                        testID={`delete-vet-note-${note.id}`}
                        accessibilityLabel="Veteriner notunu sil"
                      >
                        <Icon name="trash-2" size={16} color={colors.mutedForeground} />
                      </Pressable>
                    </View>
      ...
      <Modal visible={addConditionVisible} transparent animationType="slide" onRequestClose={closeConditionModal}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>{editingCondition ? 'Rahatsızlığı düzenle' : 'Rahatsızlık ekle'}</Text>
            <Text style={styles.modalLabel}>Başlık</Text>
            <TextInput
              style={styles.modalInput}
              value={conditionTitle}
              onChangeText={setConditionTitle}
              placeholder="Örn. Mevsimsel alerji"
              placeholderTextColor={colors.mutedForeground}
            ...
            <TextInput
              ...
              placeholder="Belirtiler, tedavi vb."
              placeholderTextColor={colors.mutedForeground}
              multiline
              testID="condition-note-input"
            />
            <View style={styles.modalActions}>
              <Pressable style={styles.modalCancel} onPress={closeConditionModal}>
                <Text style={styles.modalCancelText}>Vazgeç</Text>
              </Pressable>
              <Pressable style={styles.modalSubmit} onPress={handleSubmitCondition} disabled={savingCondition} testID="condition-submit">
                {savingCondition ? (
                  <ActivityIndicator color={colors.primaryForeground} />
                ) : (
                  <Text style={styles.modalSubmitText}>{editingCondition ? 'Kaydet' : 'Ekle'}</Text>
                )}
              </Pressable>
            </View>
          </View>
      ...
      <Modal visible={addVetNoteVisible} transparent animationType="slide" onRequestClose={closeVetNoteModal}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>{editingVetNote ? 'Veteriner notunu düzenle' : 'Veteriner notu ekle'}</Text>
            <Text style={styles.modalLabel}>Veteriner / klinik (isteğe bağlı)</Text>
            <TextInput
              style={styles.modalInput}
              value={vetNoteVetName}
              onChangeText={setVetNoteVetName}
              placeholder="Örn. Dr. Ayşe Yıldız"
              placeholderTextColor={colors.mutedForeground}
              testID="vet-note-name-input"
            />
            <Text style={styles.modalLabel}>Not</Text>
            <TextInput
              style={[styles.modalInput, styles.modalTextArea]}
              value={vetNoteText}
              onChangeText={setVetNoteText}
              placeholder="Ziyaret sırasında söylenenler"
              placeholderTextColor={colors.mutedForeground}
              multiline
              testID="vet-note-text-input"
            />
            <View style={styles.modalActions}>
              <Pressable style={styles.modalCancel} onPress={closeVetNoteModal}>
                <Text style={styles.modalCancelText}>Vazgeç</Text>
              </Pressable>
              <Pressable style={styles.modalSubmit} onPress={handleSubmitVetNote} disabled={savingVetNote} testID="vet-note-submit">
                {savingVetNote ? (
                  <ActivityIndicator color={colors.primaryForeground} />
                ) : (
                  <Text style={styles.modalSubmitText}>{editingVetNote ? 'Kaydet' : 'Ekle'}</Text>
                )}
              </Pressable>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  vaccineDeleteButton: { marginLeft: 4, padding: 6 },
  ...
  rowIconButton: { marginLeft: 4, padding: 6 },
  rowIconGroup: { flexDirection: 'row', alignItems: 'center' },
  notesTitle: { marginBottom: 12 },
  doctorRowHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  ...
  modalTextArea: { height: 88, paddingTop: 12, textAlignVertical: 'top' },
