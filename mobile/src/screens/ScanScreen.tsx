...
export default function ScanScreen() {
  ...
  const { user } = useAuth();
  const plusAccess = hasPlusAccess(user?.subscription);
  ...
  async function handleScanPress() {
    setScanning(true);
    try {
      // Görev #50: artık gerçek bir kamera fotoğrafı çekiliyor ve backend'e
      // gönderiliyor — backend'de GOOGLE_VISION_API_KEY yapılandırıldıysa
      // (bkz. backend/.env.example) gerçek görüntü tanıma denenir, yoksa
      // eski demo eşleşmesine döner (bkz. productsApi.scanProduct'ın notu).
      const image = await captureProductPhoto();
      if (!image) {
        // Kullanıcı kamerayı iptal etti — sessizce vazgeç, hata gösterme.
        return;
      }
      const product = await productsApi.scanProduct({ petId: selectedPet?.id, image });
      await loadHistory();
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView
        ...
        refreshControl={<RefreshControl testID="scan-refresh-control" refreshing={refreshingHistory} onRefresh={handleRefreshHistory} tintColor={colors.primary} />}
      >
...
const styles = StyleSheet.create({
  ...
  demoBadge: { backgroundColor: colors.card, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 8 },
  demoBadgeText: { color: colors.mutedForeground, fontFamily: fonts.body, fontSize: 10, fontWeight: '800', letterSpacing: 0.6 },
  demoBox: { backgroundColor: colors.muted, borderRadius: 12, padding: 14, marginTop: 20 },
