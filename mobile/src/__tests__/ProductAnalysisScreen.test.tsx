...
describe('ProductAnalysisScreen — Takvime ekle button', () => {
  ...
  it('calls careItemsApi.createCareItem with the product and selected pet', async () => {
    const { findByText, getByText } = await render(<ProductAnalysisScreen />);

    await findByText('OmegaPet 3');
    await fireEvent.press(getByText('Takvime ekle'));
...
describe('ProductAnalysisScreen — fiyat notları', () => {
  ...
  it('adds a price note the user logged and shows it in the list', async () => {
    const { findByText, getByTestId } = await render(<ProductAnalysisScreen />);

    await findByText('OmegaPet 3');
    await fireEvent.press(getByTestId('price-note-add-button'));

    await fireEvent.changeText(getByTestId('price-note-store-input'), 'Petshop XYZ');
    await fireEvent.changeText(getByTestId('price-note-price-input'), '249,90');
    await fireEvent.press(getByTestId('price-note-submit'));
  ...
  it('rejects an empty store name instead of saving a blank note', async () => {
    ...
    await findByText('OmegaPet 3');
    await fireEvent.press(getByTestId('price-note-add-button'));
    await fireEvent.changeText(getByTestId('price-note-price-input'), '100');
    await fireEvent.press(getByTestId('price-note-submit'));
  ...
  it('shows an alert instead of failing silently when sharing fails', async () => {
    ...
    const { findByText, getByLabelText } = await render(<ProductAnalysisScreen />);

    await findByText('OmegaPet 3');
    await fireEvent.press(getByLabelText('Paylaş'));
  ...
  it('deletes an existing price note', async () => {
    ...
    const { findByText, getByTestId, queryByTestId } = await render(<ProductAnalysisScreen />);

    await findByText('OmegaPet 3');
    await waitFor(() => getByTestId('price-note-note-1'));

    await fireEvent.press(getByTestId('price-note-delete-note-1'));
