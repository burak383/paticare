...
// Overridable via DATA_DIR so a deploy can point this at a mounted
// persistent volume (e.g. Railway's /data) instead of the container's
// own writable layer, which most hosts wipe on every redeploy — without
// this, "deployed" would silently mean "resets to empty on every push".
// Unset (local dev, `npm start`) keeps the original relative path.
