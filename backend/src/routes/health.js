...
router.patch('/health/conditions/:id', (req, res) => {
  const record = db.find('conditions', (c) => c.id === req.params.id);
  if (!record) return res.status(404).json({ error: 'Rahatsızlık kaydı bulunamadı.' });
  const pet = ownedPet(req, record.petId);
  if (!pet) return res.status(404).json({ error: 'Rahatsızlık kaydı bulunamadı.' });
  const allowed = ['title', 'note'];
  const patch = {};
  allowed.forEach((key) => {
    if (key in (req.body || {})) patch[key] = req.body[key];
  });
  const updated = db.update('conditions', record.id, patch);
  res.json({ condition: updated });
});

router.delete('/health/conditions/:id', (req, res) => {
  const record = db.find('conditions', (c) => c.id === req.params.id);
  if (!record) return res.status(404).json({ error: 'Rahatsızlık kaydı bulunamadı.' });
  const pet = ownedPet(req, record.petId);
  if (!pet) return res.status(404).json({ error: 'Rahatsızlık kaydı bulunamadı.' });
  db.remove('conditions', record.id);
  res.json({ success: true });
});
...
router.patch('/health/vet-notes/:id', (req, res) => {
  const record = db.find('vetNotes', (n) => n.id === req.params.id);
  if (!record) return res.status(404).json({ error: 'Veteriner notu bulunamadı.' });
  const pet = ownedPet(req, record.petId);
  if (!pet) return res.status(404).json({ error: 'Veteriner notu bulunamadı.' });
  const allowed = ['vetName', 'note', 'date'];
  const patch = {};
  allowed.forEach((key) => {
    if (key in (req.body || {})) patch[key] = req.body[key];
  });
  const updated = db.update('vetNotes', record.id, patch);
  res.json({ vetNote: updated });
});

router.delete('/health/vet-notes/:id', (req, res) => {
  const record = db.find('vetNotes', (n) => n.id === req.params.id);
  if (!record) return res.status(404).json({ error: 'Veteriner notu bulunamadı.' });
  const pet = ownedPet(req, record.petId);
  if (!pet) return res.status(404).json({ error: 'Veteriner notu bulunamadı.' });
  db.remove('vetNotes', record.id);
  res.json({ success: true });
});
